# Wissensdatenbank

Willkommen in meiner PARA-Wissensdatenbank.

## PARA Struktur

| Ordner | Zweck | Beispiel |
|---|---|---|
| [[0 Inbox/Inbox]] | Schnelleingang fuer noch nicht sortierte Notizen | Ideen, neue Aufgaben |
| [[1 Projects/Projects]] | Aktive Vorhaben mit konkretem Ziel | Website-Relaunch, Umzug |
| [[2 Areas/Areas]] | Laufende Verantwortungsbereiche | Gesundheit, Finanzen |
| [[3 Resources/Resources]] | Themen und Referenzwissen | Tech-Dokumentation |
| -> | -> | [[PARA Methode]] |
| [[4 Archives/Archives]] | Abgeschlossenes und Inaktives | Alte Projekte |

## Neue Notiz

- Fuege Notizen in `0 Inbox` hinzu
- Oder nutze die Templates aus `Templates/`
- Spaeter in die richtige PARA-Kategorie verschieben
